...
@Component(service = PermissionSyncService.class)
public class PermissionSyncServiceImpl implements PermissionSyncService {

    private static final Logger log = LoggerFactory.getLogger(PermissionSyncServiceImpl.class);

    // ---------------------------------------------------------
    // Process ACLs, expand groups, map users, copy ACEs
    // ---------------------------------------------------------
    @Override
    public void syncFolderPermissions(Session session, ResourceResolver resolver, String folderPath)
            throws RepositoryException {

        log.info("Starting permission sync for folder: {}", folderPath);
        ...
        log.info("Permission sync completed for folder: {}", folderPath);
